...
async function buildOrgDashboard(req, res) {
  ...
  const [baseStats, feed, departments] = await Promise.all([
    getDashboardStats(newEmployeesPeriod),
    buildActivityFeedForViewer(req.user, { limit: RECENT_ACTIVITY_LIMIT }),
    getDepartmentBreakdown(),
  ])
  ...
  res.json({
    ...
    metrics: primaryMetrics,
    primaryMetrics,
    activities: await mapActivityRowsAsync(feed.rows, feed.viewer),
    newEmployeesPeriod,
    departments,
  })
}
...
async function buildEmployeeDashboard(req, res) {
  ...
  const [stats, feed, isTeamLead] = await Promise.all([
  ...
  res.json({
    ...
    primaryMetrics,
    secondaryMetrics,
    activities: (await mapActivityRowsAsync(feed.rows, viewer)).slice(
      0,
