...
/** Indian city/state samples for seed address fields. */
const ADDRESS_LOCATIONS = [
  { cityState: 'Bengaluru, Karnataka', postalCode: '560001' },
  { cityState: 'Mumbai, Maharashtra', postalCode: '400001' },
  { cityState: 'Pune, Maharashtra', postalCode: '411001' },
  { cityState: 'Hyderabad, Telangana', postalCode: '500001' },
  { cityState: 'Chennai, Tamil Nadu', postalCode: '600001' },
  { cityState: 'New Delhi, Delhi', postalCode: '110001' },
  { cityState: 'Gurugram, Haryana', postalCode: '122001' },
  { cityState: 'Noida, Uttar Pradesh', postalCode: '201301' },
  { cityState: 'Ahmedabad, Gujarat', postalCode: '380001' },
  { cityState: 'Jaipur, Rajasthan', postalCode: '302001' },
  { cityState: 'Kochi, Kerala', postalCode: '682001' },
  { cityState: 'Kolkata, West Bengal', postalCode: '700001' },
]

function addressFor(index) {
  const location = ADDRESS_LOCATIONS[Math.abs(index) % ADDRESS_LOCATIONS.length]
  return {
    country: 'India',
...
function withAddress(employee, index) {
  return {
    ...employee,
    ...addressFor(index),
  }
...
function buildEmployees() {
  ...
  for (const anchor of ANCHORS) {
    if (!anchor.departmentId) continue
  ...
  const usedEmails = new Set(
    ANCHORS.filter((row) => row.departmentId).map((row) => row.email),
  )
  ...
  const employees = DEPARTMENT_TARGETS.flatMap((dept) => byDept[dept.id]).map(
    (employee, index) => withAddress(employee, index),
  )
  if (employees.length !== 105) {
    throw new Error(`Expected 105 employees, built ${employees.length}`)
  }

  const adminAnchor = ANCHORS.find((row) => row.id === 'EMP-1')
  if (adminAnchor) {
    employees.push(withAddress({ ...adminAnchor }, employees.length))
  }
...
export async function seedEmployees() {
  ...
  // Backfill any leftover rows (e.g. manually created) still missing address.
  const missing = await query(
    `SELECT id FROM employees
     WHERE country IS NULL OR btrim(country) = ''
        OR city_state IS NULL OR btrim(city_state) = ''
        OR postal_code IS NULL OR btrim(postal_code) = ''
     ORDER BY id`,
  )
  for (let i = 0; i < missing.rows.length; i += 1) {
    const address = addressFor(i + 200)
    await query(
      `UPDATE employees
       SET country = $2,
           city_state = $3,
  ...
  console.log(
    `Seeded ${employees.length} employees (addresses filled; ${missing.rows.length} backfilled)`,
  )
